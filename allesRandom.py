...
class Grid(object):
    def __init__(self, width, height, humnum, mosnum,
                 ninfected, netchance, chancetodie,
                 chancetodiesick, chancetogetimmune, chancetoloseimmunity):
        self.width = width
        self.height = height
        size = width*height
...
##        self.chancetodie = chancetodie
##        self.chancetodiesick = chancetodiesick
##        self.chancetogetimmune = chancetogetimmune
##        self.chancetoloseimmunity = chancetoloseimmunity
        randList = random.sample(self.tileNumberList, humnum)
        for i in range(humnum):
            humanList.append(Human(randList[i], 1, chancetodie, chancetodiesick, chancetogetimmune, chancetoloseimmunity))
    ...
    def update(self):
        ...
        for human in self.humanList:
            ...
            if human.die():
                ...
                newHuman = Human(self.randomPosition(), human.net, human.chancetodie,
                                 human.chancetodiesick, human.chancetogetimmune, human.chancetoloseimmunity)
...
class Human(object):    
    def __init__(self, position, net, chancetodie, chancetodiesick, chancetogetimmune, chancetoloseimmunity):
    ...
    def update(self):
        ...
        if self.sick == 1:
            self.daysSick +=1
            if random.random()*math.exp(-self.daysSick/100000) < self.chancetogetimmune:
                self.sick = 0
                self.immune = 1
                self.daysSick = 0   
        #lose immunity
        else:
            if random.random() < self.chancetoloseimmunity:
                self.immune = 0

    def die(self):
        
        if self.sick == 1 and random.random() < self.chancetodiesick:
          return True
        elif random.random() < self.chancetodie:
          return True
...
def runSim(duration, width, height, humnum, mosnum, ninfected, netchance, chancetodie, chancetodiesick, chancetogetimmune, chancetoloseimmunity):
    grid = Grid(width, height, humnum, mosnum, ninfected, netchance, chancetodie, chancetodiesick, chancetogetimmune, chancetoloseimmunity)
...
runSim(duration = 501, width = 200, height = 200,
       ...
       netchance = 0.5, chancetodie = 0.00001, chancetodiesick = 0.001,
